...
function list_emprunt_membre($id_membre) {
    $db = dbconnect();
    $sql = "SELECT e.*, o.nom_objet, c.nom_categorie, i.nom_image
            FROM emprunt e
            JOIN objet o ON e.id_objet = o.id_objet
            JOIN categorie_objet c ON o.id_categorie = c.id_categorie
            LEFT JOIN (
                SELECT id_objet, MIN(id_image) as id_image
                FROM images_objet
                GROUP BY id_objet
            ) AS first_img ON o.id_objet = first_img.id_objet
            LEFT JOIN images_objet i ON first_img.id_image = i.id_image
...
function nb_total_emprunts_rendu () {
        $req1 = mysqli_query(dbconnect(), "SELECT COUNT(*) AS total FROM etat_retour");
    $res = array();
    while ($result1 = mysqli_fetch_array($req1)) {
        $res[] = $result1;
    }
...
function ok () {
        $req1 = mysqli_query(dbconnect(), "SELECT COUNT(*) AS total FROM etat_retour WHERE etat = 'intact'");
    $res = array();
    while ($result1 = mysqli_fetch_array($req1)) {
        $res[] = $result1;
    }
...
function simba() {
        $req1 = mysqli_query(dbconnect(), "SELECT COUNT(*) AS total FROM etat_retour WHERE etat = 'abime'");
    $res = array();
    while ($result1 = mysqli_fetch_array($req1)) {
        $res[] = $result1;
    }
